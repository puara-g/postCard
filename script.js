...
function render(res, meta){
  ...
  if(lackIdx>=0 && lackIdx!==dominantElemIdx){ ohengPara += " " + OHENG_LACK_TEXT[ELEM_VARS[lackIdx]]; }
...
function renderToday(){
  ...
  if(lastChart.lackIdx>=0 && todayBranchElemIdx===lastChart.lackIdx){
    paras.push(ELEM_TODAY_FILL[ELEM_VARS[todayBranchElemIdx]]);
  } else if(todayBranchElemIdx===lastChart.dominantElemIdx){
    paras.push(ELEM_TODAY_OVER[ELEM_VARS[todayBranchElemIdx]]);
  }
...
const COMPAT_TEXT = {
  "비겁": (A,B)=>`${withWa(A)} ${withNeun(B)} 서로 비슷한 기운을 타고났어요. 통하는 게 많아서 편안하지만, 취향도 고집도 겹쳐서 가끔은 부딪힐 수 있어요.`,
  "식상": (A,B)=>`${A}의 기운이 자연스럽게 ${withReul(B)} 북돋아주는 관계예요. ${withGa(A)} 이끌고 ${withGa(B)} 받아들이면 편안하게 잘 맞아요.`,
  "재성": (A,B)=>`${withGa(A)} 은근히 주도권을 쥐는 관계예요. ${withNeun(A)} 이끌고 ${withNeun(B)} 맞춰주는 흐름이 자연스러운 편이에요.`,
  "관성": (A,B)=>`${withGa(B)} ${withE(A)} 자극과 긴장감을 주는 관계예요. 덕분에 서로 자라나는 사이가 될 수 있어요.`,
  "인성": (A,B)=>`${B}의 기운이 ${withReul(A)} 든든하게 채워주는 관계예요. ${withGa(A)} ${withE(B)} 편하게 기댈 수 있는 사이예요.`
};
